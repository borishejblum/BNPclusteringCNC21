...
To assess convergence, we can run different chains and compare the posterior similarity matrices or traces across chains. 
...
# Prior for alpha
Let's look closer at the priors, especially the concentration parameter. By default, $\alpha$ is random. Run the model setting $\alpha$ to 20.
...
Different label switching modes are available, in order to assure a good mixing of the classes. The first move (123) ...
...
for(i in 1:length(whichLabelSwitch)){
  mod<-profRegr(yModel="Normal", # model type for the outcome
                xModel="Mixed", # model type for the covariates
                nSweeps=10000, 
                nClusInit=20,
                nBurn=2000, 
...
We want to compare the clusterings:
